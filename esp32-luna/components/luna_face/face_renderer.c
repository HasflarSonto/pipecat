        ...
        if (xSemaphoreTake(s_renderer.mutex, pdMS_TO_TICKS(10)) == pdTRUE) {
            if (s_renderer.mode == DISPLAY_MODE_FACE) {
                ...
                if (now - s_timer_last_tick >= 1000) {
                    ...
                    // Update time text and arc - minimal updates to avoid artifacts
                    ...
                    if (bsp_display_lock(100)) {
                        ...
                        // Update arc progress
                        if (s_timer_arc && s_timer_total_seconds_start > 0) {
                            int current_seconds = s_timer_minutes * 60 + s_timer_seconds;
                            int arc_angle = (current_seconds * 360) / s_timer_total_seconds_start;
...
// Timer widgets (s_timer_arc declared at top of file)
                               ...
                               bool is_running)
{
    ...
    if (xSemaphoreTake(s_renderer.mutex, pdMS_TO_TICKS(200)) == pdTRUE) {
        ...
        if (bsp_display_lock(500)) {
            ...
            // Set initial arc angle based on current time
            int current_seconds = minutes * 60 + seconds;
            int arc_angle = (s_timer_total_seconds_start > 0) ?
                (current_seconds * 360) / s_timer_total_seconds_start : 360;
            lv_arc_set_angles(s_timer_arc, 0, arc_angle);
            lv_obj_set_style_arc_color(s_timer_arc, lv_color_hex(COLOR_ACCENT_GREEN), LV_PART_INDICATOR);
