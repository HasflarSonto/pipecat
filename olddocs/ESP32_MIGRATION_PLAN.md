...
## Known Issues & Developer Handoff

**See `ESP32_DEV_HANDOFF.md` for detailed frontend development documentation.**

### Face Renderer Issues

The following issues exist in the current ESP32 face renderer implementation:

| Issue | Status | Location | Description |
|-------|--------|----------|-------------|
| **Smile/Frown same as Neutral** | Open | `face_renderer.c:367-418` | Mouth always appears as a horizontal line regardless of emotion. The 25-pixel offset may not be visually noticeable. |
| **Cat Face not distinctive** | Open | `face_renderer.c:389-394` | Just wider line positioned lower. Needs whiskers, cat ears, or eye changes. |
| **Sparkle/Excited untested** | Open | `face_renderer.c:335-359` | Small sparkle circles exist but need verification. |

### Why Arc Widgets Were Abandoned

Previously tried using `lv_arc` for curved smiles/frowns:
1. White line artifacts accumulated on display
2. Arcs didn't render for some configurations
3. SPI queue overflow when updating arc parameters

Current implementation uses line positioning instead (simpler but less expressive).

### Backend Status

The Pipecat backend (`my_bot.py`) has ESP32 transport processors implemented:
- `ESP32AudioInputProcessor` - bridges WebSocket audio to pipeline
- `ESP32AudioOutputProcessor` - bridges TTS audio back to ESP32
- Full pipeline: Audio Input → VAD → STT → LLM → TTS → Audio Output

---

**Last Updated**: January 2025
**Status**: Implementation In Progress - Backend Ready, Frontend Needs Face Fixes
