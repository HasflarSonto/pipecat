...
- **Change detection for paired elements**: When optimizing with "last value" tracking to skip unchanged updates, track ALL independent elements. Example bug: tracking only `last_eye_h` (left) but not right eye height caused right eye to stop updating when its wink animation completed, because `eye_changed` became false. Always track both elements in a pair (left/right, top/bottom) separately.
- **Mode switching state reset**: When switching display modes, call `hide_all_screen_elements()` INSIDE the `bsp_display_lock()` block, and reset ALL animation state (wink, petting, offsets) to prevent ghost artifacts and stuck states.
- **Mutex deadlock in render loop**: When the render task needs to call functions that take `s_renderer.mutex` (like `face_renderer_set_dizzy`), use internal helper functions that bypass the mutex (e.g., `set_dizzy_internal`). The render task already holds the mutex, so trying to take it again causes a deadlock/timeout. Example: dizzy auto-timeout in `update_animation()` must call `set_dizzy_internal(false)` instead of `face_renderer_set_dizzy(false)`.
- **DO NOT set screen background in hide_all_screen_elements()**: Setting `lv_obj_set_style_bg_color(scr, ...)` or `lv_obj_set_style_bg_opa(scr, ...)` inside `hide_all_screen_elements()` causes all rendering to be stuck in the top-left corner of the screen. The background is set correctly during init and should not be modified during mode switches.
- **DO NOT use lv_obj_invalidate() in hide_all_screen_elements()**: Using `lv_obj_invalidate(scr)` to force full screen redraw also causes the top-left corner rendering issue.
- **Page switching screen clear**: LVGL partial refresh doesn't clear stale pixels when widgets are hidden. To force a full screen clear when switching modes, create a temporary full-screen opaque panel, call `lv_refr_now(NULL)` to force immediate refresh, then delete the panel. This "paints" the entire screen with the background color before new widgets are drawn. See `hide_all_screen_elements()` for implementation.
- **Move face widgets off-screen when hiding**: In addition to setting `LV_OBJ_FLAG_HIDDEN`, move main face widgets (eyes, mouth_arc, cat_arcs, wavy_mouth) to position (-200, -200). This helps LVGL mark the old position as dirty for redraw.
